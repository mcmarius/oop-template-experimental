...
/* struct timeval definition.
   This mock header is only used in CI builds where real libpcap is not available.
   We unconditionally define it here since there's no real system pcap.h to conflict with. */
struct timeval {
    long tv_sec;
    long tv_usec;
};
